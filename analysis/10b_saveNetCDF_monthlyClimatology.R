...
for(ic in 1:length(climatology)){
  ...
  climYears <- climatology[[ic]][['climatologyYears']]
  # make a matrix of each variable by depth (rows) and month (columns) ----
  ## get unique mons for columns ----
  umon <- sort(unique(df[['month']]))
  ## get unique depths for rows ----
  udepth <- sort(unique(df[['pressure']]))
  ## get list of variables ----
  vars <- names(df)[!names(df) %in% c('month', 'pressure')]
  ## define output ----
  ...
  for(iv in 1:length(vars)){
    ...
    data <- df[[var]]
    # define matrix ----
    mvars[[iv]] <- matrix(data = NA, nrow = length(udepth), ncol = length(umon))
    for(imon in 1:length(umon)){ # rows
      lookmon <- umon[imon]
      okrow <- df[['month']] == lookmon
      for(idep in 1:length(udepth)){ # columns
        lookdepth <- udepth[idep]
        okcol <- df[['pressure']] == lookdepth
        okdata <- which(okrow & okcol)
        mvars[[iv]][idep, imon] <- data[okdata]
      } # closes idep
    } # closes imon
  } # closes vars
  ...
  # note : because this is a monthly climatology and the time variables are a bit different
  #        I'm following example 7.10 of https://cfconventions.org/cf-conventions/cf-conventions.html#climatological-statistics
  ## define dimensions ----
  ### pressure (depth) ----
  ...
  dimtime <- ncdim_def(name = 'time',
                       ...
                       calendar = 'standard')
  ### nv for climatology bounds (for time)
  ...
  ncatt_put(con, varT, 'cell_method', 'time: mean over years') # for climatology calculation (see note below 'set up netCDF output' code section header)
  ...
  ncatt_put(con, varTSD, 'cell_method', 'time: standard deviation over years')
  ## salinity ----
  ...
  ncatt_put(con, varS, 'cell_method', 'time: mean over years')
  ...
  ncatt_put(con, varSSD, 'cell_method', 'time: standard deviation over years')
  ## sigmaTheta ----
  ...
  ncatt_put(con, varSTSD, 'cell_method', 'time: standard deviation over years')
  ## time
