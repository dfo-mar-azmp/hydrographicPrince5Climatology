...
## re-name b/c output variable name is same for monthly and daily file
mixedLayerDepthMonthlyClimatology <- mixedLayerDepthClimatology
# load daily file
...
ylims <- lapply(vars, function(k)
  rev(range(unlist(lapply(mixedLayerDepthClimatology, function(kk)
    kk[['dailyData']][k])), na.rm = TRUE)))
names(ylims) <- vars
# get number of observation limit
nlim <- c(0,
          max(unlist(lapply(mixedLayerDepthClimatology, function(k) unlist(lapply(vars, function(kkk) as.vector(table(k[['dailyData']][['yearDay']][!is.na(k[['dailyData']][[kkk]])]))))))))
nlimmonth <- c(0,
               max(unlist(lapply(mixedLayerDepthClimatology, function(k) unlist(lapply(vars, function(kkk) as.vector(table(k[['dailyData']][['month']][!is.na(k[['dailyData']][[kkk]])]))))))))
# calculate difference between climatology periods
## daily
climMinYear <- unlist(lapply(mixedLayerDepthClimatology, function(k) min(k[['climatologyYears']])))
## want the max year minus the min year
ok1 <- which.max(climMinYear)
ok2 <- which.min(climMinYear)
df1 <- mixedLayerDepthClimatology[[ok1]][['dailyClimatologyDf']]
df2 <- mixedLayerDepthClimatology[[ok2]][['dailyClimatologyDf']]
...
## monthly
climMinYear <- unlist(lapply(mixedLayerDepthMonthlyClimatology, function(k) min(k[['climatologyYears']])))
## want the max year minus the min year
ok1 <- which.max(climMinYear)
ok2 <- which.min(climMinYear)
df1 <- mixedLayerDepthMonthlyClimatology[[ok1]][['monthlyClimatologyDf']]
df2 <- mixedLayerDepthMonthlyClimatology[[ok2]][['monthlyClimatologyDf']]
...
for(var in vars){
  ...
  difflim <- max(abs(c(unlist(lapply(c(0.5, -0.5), function(k) dailyDifference[['mean']] + (k*dailyDifference[['sd']]))),
                 ...
                 na.rm = TRUE) * c(-1, 1)
  # initiate png output
  filename <- '09a_mixedLayerDepthClimatology.png'
  ...
  varlabel <- switch(var,
                     ...
                     'mixedLayerDepthDefault' = bquote('Mixed layer depth' ~ .(L) * m * .(R)))
  # plot daily nobs and fits
  for(ic in 1:length(mixedLayerDepthClimatology)){
    d <- mixedLayerDepthClimatology[[ic]]
    # get the daily data
    dd <- d[['dailyData']]
    # get the fitted data
    ...
    ## label
    if(ic == length(mixedLayerDepthClimatology)){
    ...
    mtext(text = paste(range(mixedLayerDepthClimatology[[ic]][['climatologyYears']]), collapse = ' to '),
    ...
    # x-axis
    ## same as barplot
    #axis(side = 1, at = label, labels = ifelse(ic == length(mixedLayerDepthClimatology), TRUE, FALSE))
    ...
    mtext(text = varlabel,
          side = 2, line = 2.3,
          cex = mtextcex)
  } # closes mixedLayerDepthClimatology (daily)
  ...
  for(imc in 1:length(mixedLayerDepthMonthlyClimatology)){
    d <- mixedLayerDepthMonthlyClimatology[[imc]]
    # get the daily data
    dd <- d[['dailyData']]
    # get the fitted data
    ...
    # x-axis
    ## add axis
    axis.POSIXct(side = 1, at = fakeX, format = '%b', labels = ifelse(imc == length(mixedLayerDepthMonthlyClimatology), TRUE, FALSE))
    if(imc == length(mixedLayerDepthMonthlyClimatology)){
      mtext(text = 'Month',
            side = 1, line = 2.3,
            cex = mtextcex)
    }
    ...
    mtext(text = varlabel,
          side = 2, line = 2.3,
          cex = mtextcex)
  } # closes mixedLayerDepthMonthlyClimatology
