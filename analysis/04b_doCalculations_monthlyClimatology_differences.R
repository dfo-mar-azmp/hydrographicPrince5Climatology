...
sctd81 <- climatology[[which(climatologyMinYear == 1981)]][['monthlyClimatologyCtd']]
sctd91 <- climatology[[which(climatologyMinYear == 1991)]][['monthlyClimatologyCtd']]
...
for(i in 1:length(sctd91)){
  ...
  if(length(oko) == 0){
    ...
    # calculate anomaly (difference), and the anomaly sd (standard deviation difference) for variables and add to nctd
    for(var in vars){
      anomaly <- nctd[[var]][oknp] - octd[[var]][okop]
      nctd <- oceSetData(object = nctd,
                         name = paste0(var, 'Anomaly'),
                         value = anomaly)
      anomalysumsqsd <- (nctd[[paste0(var, 'SD')]][oknp])^2 + (octd[[paste0(var, 'SD')]][okop])^2
      anomalysd <- sqrt(anomalysumsqsd)
      nctd <- oceSetData(object = nctd,
                         name = paste0(var, 'AnomalySD'),
                         value = anomalysd)
    }
...
climatologyWAnomaly[[1]][['monthlyClimatologyCtd']] <- sctd91WAnomaly
...
climatology <- c(climatology,
                 climatologyWAnomaly)
save(climatology, file = paste(destDirData, 'monthlyClimatologyWAnomaly.rda', sep = '/'))
