...
png(filename = paste(destDirFigures,
                     '06b_sectionMonthlyClimatologyAndDifference.png',
                     sep = '/'),
...
xlim <- as.POSIXct(range(unlist(lapply(climatology[[1]][['monthlyClimatologyCtd']], '[[', 'startTime'))),
                   origin = '1970-01-01',
                   tz = 'UTC')
ylim <- rev(range(climatology[[1]][['monthlyClimatologyCtd']][[1]][['pressure']]))
for(ic in 1:2){
  cctd <- climatology[[ic]][['monthlyClimatologyCtd']]
  # create section
  s <- as.section(cctd)
